...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="otaSyncPolicyList" @selection-change="handleSelectionChange">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listOtaSyncPolicy(this.addDateRange(this.queryParams, this.dateRange)).then(
          responseData => {
            const response = responseData.data;
            this.otaSyncPolicyList = response.records;
      ...
      handleStatusChange(row) {
        ...
        this.$confirm('确认要"' + text + '""' + row.otaCode + '"编码吗?', "警告", {
          ...
          return changeOtaSyncPolicyStatus(row.id, row.status);
        }).then(() => {
      ...
      handleUpdate(row) {
        ...
        getOtaSyncPolicy(id).then(response => {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            ...
            if (this.form.id != undefined) {
              updateOtaSyncPolicy(this.form).then(response => {
              ...
              addOtaSyncPolicy(this.form).then(response => {
                if (response.code === 200) {
